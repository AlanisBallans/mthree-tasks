...
public class Factorial {
	 ...
	 public int isInputNumberValid()  {
		...
		*/
		...
		// Returns error if non-integer value entered
		try {
		...
		// Returns error if integer is out of range
		if (num < 1 || num > 10) {
			this.printStream.print(errorMessage);
			return -1;
		}
	 ...
	 private void calculateFactorial(int num) {
		...
		*/
		...
		//YOUR CODE STARTS HERE
		 ...
		 // 1 factorial is already calculated, so continues to multiply result up from 2 to reduce unnecessary loops
		 for (int i = 2; i <= num; i++) {
			 result *= i;
		 }
